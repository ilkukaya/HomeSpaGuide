---
asin: B0BRD89BCY
slug: intex-purespa-greywood
title: Intex PureSpa Plus Greywood Deluxe Inflatable Hot Tub
shortTitle: Intex PureSpa Plus Greywood
brand: intex
category: inflatable-hot-tubs
subcategories: [bubble-jet, 4-person]
collections: [best-under-1000]
shortDescription: Intex's flagship PureSpa with 170 bubble jets, a hard-water treatment system, and the cleanest control panel in the segment.
description: |
  The Greywood Deluxe is what the inflatable category looks like when a manufacturer iterates over a decade. The Fiber-Tech beam construction sits straighter than competitors, the integrated hard-water mineral treatment really does cut chemical use by roughly a third, and the LED-lit control panel makes overnight temperature checks a non-issue.
keyFeatures:
  - 4-adult capacity (77" diameter, 28" deep)
  - 170 PureSpa bubble jets — most in class
  - Built-in hard-water treatment
  - LED-lit touch control panel
  - Insulated cover with locking buckles
  - Integrated headrest pillows (2 included)
pros:
  - Highest jet count of any inflatable in this comparison
  - Hard-water system genuinely reduces chemical demand
  - Solid-feeling Fiber-Tech beam construction
  - Quietest control panel buttons in the segment
cons:
  - Premium pricing vs. competition
  - Pump housing is bulky to store off-season
  - Headrests are functional but soft
bestFor:
  - Buyers who want the most polished inflatable
  - Hard-water households
  - Anyone who values night-time usability
specs:
  capacity: 4
  dimensions: '77" x 77" x 28"'
  weight: 99 lbs
  powerRequirement: 110V – 120V
  waterCapacity: "210 gallons"
  jetCount: 170
  heaterWattage: 1300
  setupType: inflatable
  indoorOutdoor: both
  seasonality: all-season
  materials: [Fiber-Tech 3-ply laminated PVC]
  warranty: "1-year limited"
priceRange: 500-1000
priceUpdatedAt: 2026-04-01
editorScore:
  overall: 8.7
  quality: 9
  value: 8
  setup: 8.5
  durability: 8.5
  energyEfficiency: 8
  notes: The most refined inflatable on the market — small touches add up.
editorPick: true
editorBadges: [Editor Pick, Top Rated]
primaryImage: ../../assets/placeholder-hot-tub.svg
imageAlt: Intex PureSpa Plus Greywood Deluxe inflatable spa
amazonUrl: https://www.amazon.com/dp/B0BRD89BCY
relatedProducts:
  - coleman-saluspa-tahiti-airjet
  - bestway-saint-tropez-hydrojet
alternativeProducts:
  - coleman-saluspa-tahiti-airjet
  - bestway-saint-tropez-hydrojet
status: active
publishedAt: 2026-04-15
updatedAt: 2026-05-02
---

Intex has been refining the PureSpa formula for over a decade and the Greywood Deluxe is the version where everything lines up. Comparing the published specs side by side, the Fiber-Tech construction is rated stiffer than competitor I-Beam designs, and Intex's hard-water mineral system is positioned to let households stretch chemical doses meaningfully — owner reports on Amazon and r/hottubs consistently mention reduced sanitizer usage.

The control panel is one of the most-praised details in owner reviews: large icons, clear backlight, and easy operation in the dark.
